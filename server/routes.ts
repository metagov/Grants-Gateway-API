...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/proxy/daoip5/:system/:file', async (req, res) => {
    try {
      ...
      console.log('🔍 Checking if this is SCF 35:', req.params.file);
      
      // Special handling for SCF 35 - implement server-side retry
      if (req.params.file === 'scf_35_applications_uri.json') {
        console.log('🔄 SCF 35 failed, attempting server-side retry...');
        
        // Try direct fetch with retry
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            console.log(`📡 Server retry attempt ${attempt}/3 for SCF 35`);
            
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 15000);
            
            const retryResponse = await fetch(`https://daoip5.daostar.org/${req.params.system}/${req.params.file}`, {
              method: 'GET',
            ...
            if (retryResponse.ok) {
              ...
              console.log('✅ SCF 35 server retry successful!');
              if (!res.headersSent) {
            ...
            console.warn(`❌ Server retry attempt ${attempt} failed:`, retryError);
          }
        }
        
        console.error('💥 All server retries failed for SCF 35');
        
        // Try alternative sources as last resort
        console.log('🔄 Attempting fallback source for SCF 35...');
        const alternativeSources = [
          'https://raw.githubusercontent.com/DAOstar/daoip/main/daoip-5/stellar/scf_35_applications_uri.json',
          'https://github.com/DAOstar/daoip/raw/main/daoip-5/stellar/scf_35_applications_uri.json'
        ];
        
        for (const altUrl of alternativeSources) {
          try {
            console.log(`📡 Trying alternative source: ${altUrl}`);
            const altResponse = await fetch(altUrl);
            if (altResponse.ok) {
              const altData = await altResponse.json();
              console.log('✅ SCF 35 fallback source successful!');
              if (!res.headersSent) {
